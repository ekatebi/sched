...
class DeviceDetailAction extends Component {
  ...
  actionReboot() {
    ...
    .catch(error => {
      appAlert.error(`Failed to reboot device ${deviceName}, ${error}`);
    });
  }
  ...
  actionRestore() {
    ...
    .catch(error => {
      appAlert.error(`Failed to set factory defaults on device ${deviceName}, ${error}`);
    });
  }
  ...
  actionDelete() {
    ...
    .catch(error => {
      appAlert.error(`Failed to delete device ${deviceName}, ${error}`);
    });
  ...
  onUpdateDevice() {
    ...
    this.state.files.forEach((file) => {
      ...
      .catch(error => {
        appAlert.error(`Failed to update device ${deviceName}, ${error}`);
      });
    });
  }
  ...
  render() {
    ...
    // If the model is ZyperHD, show this element. Otherwise, hide it.
    let showHdStyle = {
      display: 'block',
      textAlign: 'center',
      margin: '7px 0px',
    };
    if (settings.status.gen.model !== 'ZyperHD') {
      showHdStyle = {
        display: 'none',
        textAlign: 'center',
        margin: '7px 0px',
    ...
    const deviceProcessed = (
      <div style={{ minWidth: 10 }}>
        ...
        <div className="row">
          <div className="col-sm-12" style={showHdStyle}>
            <div>
