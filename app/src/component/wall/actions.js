      ...
      if (decoderName) {
        ...
        if (decoders && decoders.config && decoders.config.info 
          ...
          if (decoderConfig.gen.model) {
            // substring of Zyper4K or ZyperHD: start=5, end=7 
            dispatch(receiveModel(wall.gen.name, decoderConfig.gen.model.substring(5, 7)));
          }
        } else {
  ...
  //      console.log('onGetModels', decoderName, wall);
        ...
        if (decoderName) {
          ...
          const decoderConfig = decoders.config.info.text.find((config) => {
            ...
            if (decoderConfig.gen.model) {
              // substring of Zyper4K or ZyperHD: start=5, end=7 
              wall.model = decoderConfig.gen.model.substring(5, 7);
            }
...
//          console.log('onRequestUpdateWall', JSON.stringify(json, 0, 2));
          ...
          json.responses.forEach((resp) => {
            if (resp.errors && resp.errors.length !== 0) {
              errorCount++;
              appAlert.show(`${resp.errors.join(', ')}, failed to save video wall \'${formData.name}\'`,
                { time: 0, type: 'error' });
            }
          });
          ...
          const error = json.status;
          appAlert.show(`${error}, failed to save video wall ${formData.name}`,
            { time: 0, type: 'error' });
          dispatch(receiveError(error));
        }
      })
    ...
    deleteWall(name)
      ...
      .then(json => {
        ...
        if (json.status === 'Success') {
          ...
          const error = json.status;
          appAlert.show(`${error}, failed deletinging video wall ${name}`,
            { time: 0, type: 'error' });
          dispatch(receiveError(error));
        }
      })
    ...
    joinSrcToWall(SrcMac, wallName)
      ...
      .then(json => {
        ...
        if (json.status === 'Success') {
          ...
          const error = json.status;
          appAlert.show(`${error}, failed Joining source ${SrcMac} to video wall ${wallName}`,
            { time: 0, type: 'error' });
          dispatch(receiveError(error));
        }
      })
