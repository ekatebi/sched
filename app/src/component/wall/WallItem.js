...
//    return item.settings && item.settings.gen.type === 'decoder' && !isDraggedDecoderInMatrix(props, item);
    ...
    if (!props.item.model || (item && item.settings && item.settings.gen && props.item.model !== item.settings.gen.model.substring(5, 7))) {
      return false;
    }
...
//      console.log('joinSrc', sourceMac, name);
  ...
  render() {
    ...
    const { index, item, model, showEditor, 
...
//            onEdit(item);
    ...
    if (onRequestDeleteWall && isConfig) {
      delSpan = (
    ...
    return (
      <div id={elemId} style={compStyle}
        ...
        <div style={imageBlockStyle}>
          <Row>
            <Col xs={1} smOffset={0}>
              <div style={{ marginTop: -2, marginLeft: -2, fontSize: 12 }}><strong>{item.model}</strong></div>
            </Col>
            <Col xs={4} smOffset={0} style={{ marginLeft: 5, marginTop: 12 }}>
              {connectDropTarget(              
                <div>
...
function mapStateToProps(state, props) {
  ...
  const { selectedSource } = state.device;
