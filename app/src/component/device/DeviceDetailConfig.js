...
import { FormGroup, FormControl, Radio,
...
//    this.setState({ showIcons: !popupPanelMouseDown });
  ...
  setConfigMode(mode) {
    ...
    .catch(error => {
      appAlert.error(`Failed to config ${mode} for device ${deviceName}, ${error}`);
      this.getDevice();
    });
  }

  setConfigIp(ip) {
    // console.log('In DeviceDetailConfig setConfigIp() 1');
    const { settings } = this.props;
    const deviceName = settings.config.gen.name;
    ...
    .catch(error => {
      appAlert.error(`Failed to config ip for device ${deviceName}, ${error}`);
    });
  }
  ...
  setConfigRs232(rs232) {
    ...
    .catch(error => {
      appAlert.error(`Failed to config rs232 for device ${deviceName}, ${error}`);
      this.getDevice();
    });
  }
  ...
  setVideoPort(videoPort) {
    ...
    .catch(error => {
      appAlert.error(`Failed to config video port ${videoPort} for device ${deviceName}, ${error}`);
      this.getDevice();
    });
  }
  ...
  setIconImageName(mac, iconImageName) {
    ...
    .catch(error => {
      appAlert.error(`Failed in setting icon image name for device ${deviceName}, ${error}`);
    })
    .then(() => {
      setTimeout(() => {
  ...
  render() {
    ...
    const { settings } = this.props;
    const { gen, ip, rs232, ports } = this.state;
    ...
    let videoPorts = ['hdmi'];
    if (settings.capabilities['video-port']) {
    ...
    // IP mode can be displayed as read only or a drop down list.
    let editIpModeStyle;
    let readOnlyIpModeStyle;
    if (statusGen.model === 'ZyperHD') {
      editIpModeStyle = { display: 'none ' };
      readOnlyIpModeStyle = { display: '' };
    } else {
      editIpModeStyle = {
        display: '',
        margin: '0px 0px 15px',
      };
      readOnlyIpModeStyle = { display: 'none' };
    }
...
//            height: iconHeight,
      ...
      const icons = iconPaths.map((path, index) => {
        ...
        return (
          <Row style={iconStyle(selected)} key={index}
            ...
            <Col sm={9} style={ { marginTop: 7 } }>{iconNames[index]}</Col>
          </Row>
        );
      });
    ...
    const selIconx = (
        <div className="col-sm-8 col-sm-offset-4">
          <Button onClick={() => {
            ...
            <i className={iconsCaret} style={caretStyle}></i>
          </Button>
          <Collapse in={this.state.showIcons}>
            <div>
              <Loader loaded={this.state.loaded} options={this.spinnerOptions}>
    ...
    const deviceProcessed = (
      ...
      <div style={{ minWidth: 300, paddingRight: 15 }}>
        <div className="row" style={editIpModeStyle}>
        ...
        <div className="row" style={readOnlyIpModeStyle}>
          <div className="col-sm-4" style={labelStyle}>IP Mode:</div>
          <div className="col-sm-8">
            <span>
              Link Local
