...
// Extract device information based on the device settings, status and config.
...
export class DeviceSettings {
  ...
  getVideo() {
    ...
    const { connectedEncoder } = this.config;
    if (!connectedEncoder
      || connectedEncoder.macAddr === 'none'
      || connectedEncoder.name === 'N/A'
      || connectedEncoder.connectionMode === 'disconnected') {
      return undefined;
    }
    const receivingVideo = statusConnectedEncoder.receivingVideoFromEncoder;
    // The API might report wall-fast-switched or fast-switched.
    // These are the same from the GUI's point of view.
    if (connectedEncoder.connectionMode === 'wall-fast-switched') {
      connectedEncoder.connectionMode = 'fast-switched';
    }
    return { ...connectedEncoder, receivingVideo };
  }
