...
function buildCommand(sourceName, displayName, joinConfigDetails, oldVideoConnection, analogAudioConnection, singleCmd = true) {
  ...
  // If video and downmix digital audio are both comming from the same encoder, these commands are the same.
  //
  ...
  //
  // But the video-source command is preferred.
  // The first type of join command sets hdmiDownMixFollowVideo to false, so it should be avoided where possible.
  let useVideoSource = false;
  if ((joinConfigDetails.digitalAudio && joinConfigDetails.digitalAudio.value === HDMI_DOWNMIX) &&
    ((joinConfigDetails.video && joinConfigDetails.video.value !== NONE) ||
      (oldVideoConnection !== NONE && oldVideoConnection === sourceName))) {
    useVideoSource = true;
  }
  ...
  if (joinConfigDetails.hasOwnProperty('digitalAudio') && joinConfigDetails.digitalAudio.value !== NO_CHANGE) {
    ...
    switch (joinConfigDetails.digitalAudio.value) {
      ...
      case HDMI_DOWNMIX:
        ...
        if (useVideoSource) {
          cmd += `join video-source ${displayName} hdmi-downmix-audio hdmi-out`;
          cmd += `${COMMAND_SEPARATOR}`;
        } else {
          cmd += `join ${sourceName} `;
  ...
  if (joinConfigDetails.hasOwnProperty('analogAudio') && joinConfigDetails.analogAudio.value !== NO_CHANGE) {
    ...
    switch (joinConfigDetails.analogAudio.value) {
      case HDMI_DOWNMIX:
        if (useVideoSource) {
          cmd += `join video-source ${displayName} hdmi-downmix-audio analog-out`;
          cmd += `${COMMAND_SEPARATOR}`;
        } else {
          cmd += `join ${sourceName} `;
          cmd += `${displayName} hdmi-downmix-audio`;
