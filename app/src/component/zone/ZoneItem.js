...
class ZoneItem extends Component {
  ...
  clickHandler(e) {
    ...
    if (e.target.id === imageId && !selectedSource) {
      onRequestList(item);
      onSelectZone(item);
  ...
  render() {
    ...
    const { index, item, showEditor, onRequestList, onRequestRemove, 
        ...
        <div style={editStyle}>
          ...
          <h4 style={namecontainerStyle}>
            <ToolTip placement="bottom"
              tooltip={item.name}>
            <a style={nameStyle}
               onClick={this.popupToggle}><strong>{item.name}</strong>
            </a>
            </ToolTip>
          </h4>
          {isConfig ? (<span key={2} style={ { cursor: 'pointer' } } onClick={(e) => {
            // console.log('edit item clicked!');
            e.preventDefault();
