  ...
  return (dispatch, getState) => {
    ...
    if (fetching) {
      ...
      try {
        ...
        fetchZones(zone ? zone.id : undefined)
          .then(resp => resp.json())
          .then(json => {
//            console.log('fetchZones:', json);
            if (json.success === true) {
//              console.log('fetchZones success:', json.result);
              const list = json.result;

//              console.log('zones list', zoneEx, zoneId, JSON.stringify(list, 0, 2));

              const listObject = list.reduce((acc, cur, i) => {
                  acc[cur.id] = cur;
                  return acc;
                }, {});
...
//              console.log('fetchZones fail:', json);
  ...
  if (id) {
    ...
    if (devices && devices.status && devices.status.info 
      ...
      if (devStatus && devStatus.gen && devStatus.gen.model) {
        ...
        return devStatus.gen.model.substring(5, 7);
      }
    }
  }
