...
export function onSelectMultiview(multiview) {
  return (dispatch, getState) => {
  	...
  	dispatch(deviceActions.selectSource());
  };
}
...
export function onRequestJoinMvToDisplay(mvName, decoderName) {
  return (dispatch, getState) => {
...
//        dispatch(onRequestList());
      ...
      .catch(error => {
        appAlert.error(`${error}, Failed to join ${mv} ${mvName} to display ${decoderName}`);
...
//          console.log('onRequestList list', list, JSON.stringify(list, 0, 2));
          ...
          appAlert.error(`${error}, failed to fetch multiviews, ${json}`);
          dispatch(receiveError(error));
        }

      })
      .catch(error => {
        appAlert.error(`${error}, failed to fetch multiviews`);
        dispatch(receiveError(error));
      })
      .then(() => {
//        dispatch(showSpinner(false));
...
//        console.log('onRequestDeleteItem json', JSON.stringify(json, 0, 2));
      ...
      .catch(error => {
        appAlert.error(`${error}, failed to delete ${mv} ${name}`);
//        dispatch(receiveError(error));
      })
      .then(() => {
//        dispatch(showSpinner(false));
...
//        console.log('onRequestSaveItem', JSON.stringify(json, 0, 2));
      ...
      .catch(error => {
        appAlert.error(`${error}, failed to save ${mv} ${item.gen.name}`);
//        dispatch(receiveError(error));
      })
      .then(() => {
//        dispatch(showSpinner(false));
